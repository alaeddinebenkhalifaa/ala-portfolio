...
export default function ExperienceModal({ ev, categoryLabel, onClose }) {
  const { t } = useLanguage()
      ...
      {ev && (
        <>
          ...
          <motion.div
            ...
            aria-label={ev.title}
          >
            <button className="em-close" onClick={onClose} aria-label={t('experienceModal.close')}>
            ...
            {images.length > 0 && (
              <div className="em-gallery">
                ...
                {images.length > 1 && (
                  <>
                    <button className="em-nav em-nav--prev" onClick={prev} aria-label={t('experienceModal.previousImage')}>
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                        <path d="M13 4l-6 6 6 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </button>
                    <button className="em-nav em-nav--next" onClick={next} aria-label={t('experienceModal.nextImage')}>
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                    ...
                    <div className="em-dots">
                      {images.map((_, i) => (
                        <button
                          ...
                          onClick={() => go(i)}
                          aria-label={`${t('experienceModal.image')} ${i + 1}`}
                        />
                      ))}
                    </div>
                  </>
            ...
            <div className="em-body">
              <div className="em-meta">
                <span className="em-cat">{categoryLabel}</span>
